...
class AfterLoginRedirection implements AuthenticationSuccessHandlerInterface
{
    ...
    public function onAuthenticationSuccess(Request $request, TokenInterface $token)
    {
        ...
        // If is a super admin we redirect to the super area
       if (in_array('ROLE_SUPER_ADMIN', $rolesTab, true) ){
            $redirection = new RedirectResponse($this->router->generate('super'));

        }
// If is a  admin we redirect to the super area
        elseif (in_array('ROLE_ADMIN', $rolesTab, true))
            $redirection = new RedirectResponse($this->router->generate('admin'));
        // otherwise we redirect user to the user area
        else
